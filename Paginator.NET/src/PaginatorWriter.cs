...
namespace Paginator.NET
{
	...
	{
		public static bool WritePages(this PageInfo pageInfo, IPaginatorWriter writer)
		{
			...
			int pagesCountMinusFirstLast = pi.PagesCount - (pi.GapAfterStart ? 1 : 0) - (pi.GapBeforeEnd ? 1 : 0);

			bool hasPrev = w.CanShowPrevious && currPg - 1 > 0;
			bool hasNext = w.CanShowNext && currPg + 1 <= pi.TotalPageCount;

			//  --- Write Prev Chapt ---
			if(w.Chapters && pi.GapAfterStart) {
			...
			for(int i = 0; i < pages.Length; i++) {
				...
				if(showGap && i == lastIdx && pi.GapBeforeEnd)
					w.WriteGap();
				...
				if(showGap && i == 0 && pi.GapAfterStart)
					w.WriteGap();
			}
